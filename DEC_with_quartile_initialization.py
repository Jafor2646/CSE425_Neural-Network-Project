...
# Compute quartile-based initialization points
Q1 = np.percentile(latent_np, 25, axis=0)
Q2 = np.percentile(latent_np, 50, axis=0)
Q3 = np.percentile(latent_np, 75, axis=0)
Qmean = np.mean(latent_np, axis=0)
quartile_centers = np.vstack([Q1, Q2, Q3, Qmean])

# Apply KMeans with custom init
kmeans = KMeans(n_clusters=4, init=quartile_centers, n_init=1, max_iter=300)
kmeans.fit(latent_np)

# Initialize clustering layer with KMeans centroids
...
plt.title("t-SNE of Final DEC Clusters (Quartile Init)")
plt.xlabel("t-SNE 1")
plt.ylabel("t-SNE 2")
plt.show()


# 🔍 Final Silhouette Score: 0.9858
# 📉 Davies-Bouldin Index: 0.0189